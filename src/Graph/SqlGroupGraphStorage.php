...
class SqlGroupGraphStorage implements GroupGraphStorageInterface {
  ...
  protected function loadGroupMapping($gid) {
    ...
    if ($cache = \Drupal::cache()->get($cid)) {
      ...
      $allGroups = $this->connection->select('groups', 'g')
        ->fields('g', ['id']);
      ...
      $query->condition('gg.end_vertex', $allGroups, 'IN');
      ...
      // returned to tag this cache with.
      $groups = array_merge($mapping['descendants'], $mapping['ancestors']);
      $groups[] = $gid;

      // Build the cache tags for all (deduped) groups.
      $cache_tags = [];
